...
<html>
	<head>
		<meta charset="UTF-8">
		<title>클릭하면 상세보기페이지 보여줄 ajax 파일</title>
	...
	<div id="left">	
		<fieldset>
			...
			<table class="tb2" style="border : 2px solid darkgrey; border-collapse : collapse; width : 600px">
				...
				<tr>
					<td style="width:30px; height:300px; text-align:center">내용</td>
					<td class="td2">${ board.fb_content }</td>
				</tr>
				
				<tr>
					<td class="td1">비밀번호</td>
					<td class="td2"><input style="border:none; background-color: #e2e2e2;" type="password" name="inputpwd" placeholder="수정, 삭제시 필수입력"> (회원일경우 회원님의 비밀번호입니다)</td>
				</tr>
			...
			<div style="text-align: center">
				...
				<input class="pbtn" type="button" value="삭제" onclick="del(this.form)"/>
			</div>
			</form>
            <div>
                <span><strong>Comments : ${Total}</strong></span><br>
                <strong>작성자 : <span style="color:orange">${sessionScope.memberID}</span></strong>
            </div>
            <form  method="post">
                <table>
                    <tr>
                        <td>
                            ...
                            <textarea style="resize:none; border:none; background-color: #e2e2e2; width: 600px" rows="3" cols="30"  name="com_comment" placeholder="댓글을 입력하세요"></textarea>
                        </td>
                    </tr>
                </table>
                      <div style="text-align: center">
                         <input class="pbtn" type="button" value="등록" onclick="regi(this.form,${ board.fb_idx });">
                      </div>
            </form>
            	...
            	<c:forEach var="com" items="${ comment }">
            		<form method="post">
		        		...
		        		${com.com_comment }
        				<div>
		        			<input class="pbtn" type="button" value="삭제" onclick="deletecomment(this.form,${ board.fb_idx });">
